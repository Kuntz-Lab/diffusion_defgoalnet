...
# Logging
if args.logging:
    log_dir = os.path.join(args.log_root, "retract_kidney_all_stochastic")
...
dataset_path = "/home/baothach/shape_servo_data/tanner/processed_data_stochastic"
dataset = DefGoalNetDataset(dataset_path)
...
def train(it):
    ...
    # # task_ctx = batch['kidney_angle'].to(args.device)
    # task_ctx = batch['init_pc'].to(args.device)
    # print("task_ctx.shape:", task_ctx.shape)
    # loss = model.get_loss(x, task_ctx, kl_weight=kl_weight, writer=writer, it=it)
    ...
    # for i, temp in enumerate([batch['pointcloud'][0], batch['context_pc'][0], batch['init_pc'][0]]):
    #     pcds.append(pcd_ize(temp.cpu().numpy(), color=colors[i]))    
